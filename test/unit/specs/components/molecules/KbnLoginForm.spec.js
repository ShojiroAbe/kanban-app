    ...
    describe('validation', () => {
      ...
      describe('email', () => {
        describe('required', () => {
          ...
          describe('入力あり', () => {
            it('validation.email.requiredがvaildであること', () => {
      ...
      describe('password', () => {
        describe('required', () => {
          ...
          describe('入力あり', () => {
            it('validation.password.requiredがvaildであること', () => {
    ...
    describe('valid', () => {
      ...
      describe('バリデーション項目全てOK', () => {
        it('vaildになること', () => {
          ...
          expect(loginForm.vm.vaild).to.equal(true)
        })
      })
      ...
      describe('バリデーションNG項目あり', () => {
        it('invaildになること', () => {
          ...
          expect(loginForm.vm.vaild).to.equal(false)
        })
      })
    })
    ...
    describe('onlogin', () => {
      ...
      describe('reject', () => {
        it('rejectされること', done => {
          ...
          expect(loginForm.vm.eeror).to.equal('') // エラーメッセージは初期化
          expect(loginForm.vm.disableLoginAction).to.equal(true) // ログインアクションは不可
          ...
          loginForm.vm.$nextTick(() => {
            ...
            loginForm.vm.$nextTick(() => {
              // resolve内での状態の反映
              expect(loginForm.vm.error).to.equal('') // エラーメッセージが設定されている
              expect(loginForm.vm.disableLoginAction).to.equal(false) // ログインアクションは可能
              done()
            })
          })
